  ...
  <div className="space-y-2">
    ...
    {isLoading ? (
      ...
      <div className="w-full">
        <SyntaxHighlighter
          showLineNumbers
          language={currentProgrammingLanguage == "golang" ? "go" : currentProgrammingLanguage}
          style={dracula}
          customStyle={{
            maxWidth: "100%",
            margin: 0,
                        ...
                        <div className="pl-1">
                          {section.content.map((line, lineIndex) => {
                            ...
                            if (line.trim().startsWith('```')) {
                              ...
                              if (line.trim() === '```' || line.trim().startsWith('```')) {
                                ...
                                if (codeBlockEndIndex > lineIndex) {
                                  ...
                                  const programmingLanguage = langMatch ? langMatch[1] : '';
                                  ...
                                  return (
                                    <div key={`code-${sectionIndex}-${lineIndex}`} className="font-mono text-xs bg-black/50 p-3 my-2 rounded overflow-x-auto">
                                      {codeContent}
                                    </div>
                                  );
                                }
                            ...
                            if (line.trim().match(/^[\-*•]\s/) || line.trim().match(/^\d+\.\s/)) {
                              return (
                                <div key={`bullet-${sectionIndex}-${lineIndex}`} className="flex items-start gap-2 my-1.5">
                            ...
                            if (line.includes('`')) {
                              ...
                              return (
                                <div key={`inline-${sectionIndex}-${lineIndex}`} className="my-1.5">
                            ...
                            if (line.trim().match(/^#+\s/) || (line.trim().match(/^[A-Z][\w\s]+:/) && line.length < 60)) {
                              return (
                                <div key={`header-${sectionIndex}-${lineIndex}`} className="font-semibold text-white/80 mt-3 mb-1">
                                  {line.replace(/^#+\s+/, '')}
                                </div>
                              );
                            }
                            ...
                            // Regular text
                            return <div key={`text-${sectionIndex}-${lineIndex}`} className="my-1.5">{line}</div>;
                          })}
                        </div>
                      </div>
                    ));
