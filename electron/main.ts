      ...
      if (isDev) {
        ...
        setTimeout(async () => {
          try {
            ...
            console.log("Successfully loaded development server on retry");
          } catch (error) {
            console.error("Failed to load dev server on retry:", error);
            // 显示友好的错误消息
            state.mainWindow?.webContents.loadURL(`data:text/html;charset=utf-8,
              <html>
                <head>
                  <title>开发服务器未运行</title>
                  <meta charset="utf-8">
                  <style>
                ...
                <body>
                  <h2>开发服务器未运行</h2>
                  <p>无法连接到开发服务器 (http://localhost:54321)</p>
                  <p>请确保您已运行：</p>
                  <p><code>npm run dev</code> 或单独运行 <code>vite</code></p>
                  <p>错误详情: ${errorCode} - ${errorDescription}</p>
                </body>
              </html>
            `);
          }
  ...
  if (isDev) {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      if (fs.existsSync(altPath)) {
        state.mainWindow.loadFile(altPath)
      } else {
        // 使用绝对路径
        ...
        if (fs.existsSync(rootPath)) {
          state.mainWindow.loadFile(rootPath)
        } else {
          console.error("All attempts to find index.html failed")
          // 显示错误给用户 - 确保使用utf-8字符集
          state.mainWindow.webContents.loadURL(`data:text/html;charset=utf-8,
            <html>
              <head>
                <title>加载错误</title>
              ...
              <body>
                ...
                <p>详细错误: 找不到dist/index.html</p>
                <p>应用版本: ${app.getVersion()}</p>
              </body>
            </html>
          `)
        }
...
async function initializeApp() {
  ...
  try {
    ...
    const sharedDictDir = path.join(cacheDir, 'session', 'Shared Dictionary');
    if (fs.existsSync(sharedDictDir)) {
      console.log('Found shared dictionary cache directory, cleaning...');
      const cacheFile = path.join(sharedDictDir, 'cache');
      if (fs.existsSync(cacheFile)) {
        fs.unlinkSync(cacheFile);
        console.log('Cache file removed successfully');
      }
    }
  } catch (err) {
    console.error('Error cleaning cache directory:', err);
  }
  
  // 配置app全局设置
  app.setPath('sessionData', path.join(app.getPath('userData'), 'session'));
  app.setAppUserModelId("Interview Coding Assistant")
